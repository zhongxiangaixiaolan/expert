		...
		<scroll-view class="form-scroll" scroll-y>
			...
			<view class="expert-section" v-if="expertInfo">
				...
				<view class="expert-card">
					...
					<view class="expert-info">
						...
						<text class="expert-category">{{ expertInfo.categoryName }}</text>
					</view>
					<text class="expert-price">¥{{ expertInfo.servicePrice }}/次</text>
				</view>
			</view>
				...
				<view class="form-item">
					...
					>
						<view class="picker-input">
							<text v-if="formData.expectedTime">{{ formatTime(formData.expectedTime) }}</text>
							<text v-else class="placeholder">请选择期望完成时间</text>
							<image class="picker-arrow" src="/static/icons/arrow-right.svg"></image>
						</view>
					</picker>
				</view>
			</view>
				...
				<view class="upload-area">
					<view class="uploaded-files" v-if="uploadedFiles.length > 0">
						<view 
							...
							:key="index"
						>
							<image class="file-preview" :src="file.url" mode="aspectFill"></image>
							<view class="file-remove" @click="removeFile(index)">
								<image class="remove-icon" src="/static/icons/close.svg"></image>
							</view>
						</view>
					</view>
					<view class="upload-btn" @click="chooseFiles" v-if="uploadedFiles.length < 6">
			...
			<view class="cost-section">
				...
				<view class="cost-list">
					<view class="cost-item">
						...
						<text class="cost-value">¥{{ expertInfo?.servicePrice || 0 }}</text>
					</view>
					<view class="cost-item">
			...
			<view class="agreement-section">
				<view class="agreement-item" @click="toggleAgreement">
					<image 
						class="checkbox" 
						:src="agreedToTerms ? '/static/icons/check.svg' : '/static/icons/close.svg'"
					></image>
...
import { formatTime, showError, showSuccess, chooseImage, requireAuth } from '@/utils/index'
...
const platformFee = computed(() => {
  const servicePrice = expertInfo.value?.servicePrice || 0
...
const canSubmit = computed(() => {
  return formData.taskDescription.trim().length >= 10 && 
         agreedToTerms.value && 
         !isSubmitting.value
})
...
const loadExpertInfo = async (expertId: number) => {
  try {
    ...
    // 设置默认服务名称
    formData.serviceName = result.serviceName || result.categoryName || '专业服务'
  } catch (error) {
